import Pluone from "./assets/planet-jupiter.svg";
import Plutwo from "./assets/planet-jupiter-internal.svg";
import PluTre from "./assets/geology-jupiter.png";
import { useState } from "react";
function Jupiter() {
  const [Link, setLink] = useState("https://en.wikipedia.org/wiki/Jupiter");
  const [Img, setImg] = useState(Pluone);
  const [Text, setText] = useState(
    "Jupiter is the fifth planet from the Sun and the largest in the Solar System. It is a gas giant with a mass two and a half times that of all the other planets in the Solar System combined, but less than one-thousandth the mass of the Sun."
  );
  const Structure = () => {
    setLink("https://en.wikipedia.org/wiki/Jupiter#Internal_structure");
    setImg(Plutwo);
    setText(
  ...
  const Overview = () => {
    setLink("https://en.wikipedia.org/wiki/Jupiter");
    setImg(Pluone);
    setText(
      "Jupiter is the fifth planet from the Sun and the largest in the Solar System. It is a gas giant with a mass two and a half times that of all the other planets in the Solar System combined, but less than one-thousandth the mass of the Sun."
    );
    ...
    <section className="flex flex-col">
      ...
      <div className="  grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 px-4 gap-1 py-2 ">
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          ...
          <p className="text-white text-2xl  font-semibold">9.93 Hours</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">REVOLUTION TIME :</h3>
          <p className="text-white text-2xl  font-semibold">11.86 Years</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">RADIUS :</h3>
          <p className="text-white text-2xl  font-semibold">69,911 KM</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">AVERAGE TEMP :</h3>
          <p className="text-white text-2xl  font-semibold">-108°c</p>
        </div>
      </div>
    </section>
  );
