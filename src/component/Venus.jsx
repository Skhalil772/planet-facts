import Pluone from "./assets/planet-venus.svg";
import Plutwo from "./assets/planet-venus-internal.svg";
import PluTre from "./assets/geology-venus.png";
import { useState } from "react";
function Venus() {
  const [Link, setLink] = useState("https://en.wikipedia.org/wiki/Venus");
  const [Img, setImg] = useState(Pluone);
  const [Text, setText] = useState(
    "Venus is the second planet from the Sun. It is named after the Roman goddess of love and beauty. As the brightest natural object in Earth's night sky after the Moon, Venus can cast shadows and can be, on rare occasions, visible to the naked eye in broad daylight."
  );
  const Structure = () => {
    setLink("https://en.wikipedia.org/wiki/Venus#Internal_structure");
    setImg(Plutwo);
    setText(
  ...
  const Geology = () => {
    setLink("https://en.wikipedia.org/wiki/Venus#Surface_geology");
    setImg(PluTre);
    setText(
  ...
  const Overview = () => {
    setLink("https://en.wikipedia.org/wiki/Venus");
    setImg(Pluone);
    setText(
      "Venus is the second planet from the Sun. It is named after the Roman goddess of love and beauty. As the brightest natural object in Earth's night sky after the Moon, Venus can cast shadows and can be, on rare occasions, visible to the naked eye in broad daylight."
    );
    ...
    <section className="flex flex-col justify-between">
      ...
      <div className="  grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 px-4 gap-1 py-2 ">
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          ...
          <p className="text-white text-2xl  font-semibold">243 Days</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">REVOLUTION TIME :</h3>
          <p className="text-white text-2xl  font-semibold">224.7 Days</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">RADIUS :</h3>
          <p className="text-white text-2xl  font-semibold">6,051.8 KM</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">AVERAGE TEMP :</h3>
          <p className="text-white text-2xl  font-semibold">471°c</p>
        </div>
      </div>
    </section>
  );
