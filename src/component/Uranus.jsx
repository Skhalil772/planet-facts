import Pluone from "./assets/planet-uranus.svg";
import Plutwo from "./assets/planet-uranus-internal.svg";
import PluTre from "./assets/geology-uranus.png";
import { useState } from "react";
function Uranus() {
  const [Link, setLink] = useState("https://en.wikipedia.org/wiki/Uranus");
  const [Img, setImg] = useState(Pluone);
  const [Text, setText] = useState(
    "Uranus is the seventh planet from the Sun. Its name is a reference to the Greek god of the sky, Uranus according to Greek mythology, was the great-grandfather of Ares. It has the third-largest planetary radius and fourth-largest planetary mass in the Solar System."
  );
  const Structure = () => {
    setLink("https://en.wikipedia.org/wiki/Uranus#Internal_structure");
    setImg(Plutwo);
    setText(
  ...
  const Geology = () => {
    setLink("https://en.wikipedia.org/wiki/Uranus#Atmosphere");
    setImg(PluTre);
    setText(
  ...
  const Overview = () => {
    setLink("https://en.wikipedia.org/wiki/Uranus");
    setImg(Pluone);
    setText(
      "Uranus is the seventh planet from the Sun. Its name is a reference to the Greek god of the sky, Uranus according to Greek mythology, was the great-grandfather of Ares. It has the third-largest planetary radius and fourth-largest planetary mass in the Solar System."
    );
    ...
    <section className="flex flex-col">
      ...
      <div className="  grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 px-4 gap-1 py-2 ">
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          ...
          <p className="text-white text-2xl  font-semibold">17.2 Hourss</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">REVOLUTION TIME :</h3>
          <p className="text-white text-2xl  font-semibold">84 Years</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">RADIUS :</h3>
          <p className="text-white text-2xl  font-semibold">25,362 KM</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">AVERAGE TEMP :</h3>
          <p className="text-white text-2xl  font-semibold">-195°c</p>
        </div>
      </div>
    </section>
  );
