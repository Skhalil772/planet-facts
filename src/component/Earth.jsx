import Pluone from "./assets/planet-earth.svg";
import Plutwo from "./assets/planet-earth-internal.svg";
import PluTre from "./assets/geology-earth.png";
import { useState } from "react";
function Earth() {
  const [Link, setLink] = useState("https://en.wikipedia.org/wiki/Earth");
  const [Img, setImg] = useState(Pluone);
  const [Text, setText] = useState(
    "Third planet from the Sun and the only known planet to harbor life. About 29.2% of Earth's surface is land with remaining 70.8% is covered with water. Earth's distance from the Sun, physical properties and geological history have allowed life to evolve and thrive."
  );
  const Structure = () => {
    setLink("https://en.wikipedia.org/wiki/Earth#Internal_structure");
    setImg(Plutwo);
    setText(
  ...
  const Geology = () => {
    setLink("https://en.wikipedia.org/wiki/Earth#Surface");
    setImg(PluTre);
    setText(
  ...
  const Overview = () => {
    setLink("https://en.wikipedia.org/wiki/Earth");
    setImg(Pluone);
    setText(
      "Third planet from the Sun and the only known planet to harbor life. About 29.2% of Earth's surface is land with remaining 70.8% is covered with water. Earth's distance from the Sun, physical properties and geological history have allowed life to evolve and thrive."
    );
    ...
    <section className="flex flex-col">
      ...
      <div className="  grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 px-4 gap-1 py-2 ">
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          ...
          <p className="text-white text-2xl  font-semibold">0.99 Days</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">REVOLUTION TIME :</h3>
          <p className="text-white text-2xl  font-semibold">365.26 Days</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">RADIUS :</h3>
          <p className="text-white text-2xl  font-semibold">6,371 KM</p>
        </div>
        <div className="border-[1px] border-gray-500 rounded-sm py-3 pl-2 pr-6 w-full mx-auto  container justify-center items-center space-x-4 md:space-x-0  md:space-y-1 flex md:flex-col">
          <h3 className="text-gray-500 text-xs ">AVERAGE TEMP :</h3>
          <p className="text-white text-2xl  font-semibold">16°c</p>
        </div>
      </div>
    </section>
  );
